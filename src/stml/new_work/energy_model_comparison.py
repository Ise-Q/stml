"""energy_model_comparison.py — Variant comparison for energy instruments.

Phase 1: CPCV (n_groups=6, k=2, embargo=0.01) on TRAIN only.
         Lock variant per instrument by 1SE rule.
Phase 2: Full-train refit, single-shot OOS on sealed test.

Instrument families (champion):
    cl1s   – XGB (individual model, SIGNAL)
    ho1s   – Logistic energy_all pool (NO SIGNAL, exploratory)
    rb1s   – Logistic energy_all pool (NO SIGNAL, exploratory)
    ng1s   – RF     energy_all pool (NO SIGNAL, below-random, exploratory)

Pooled-model mechanics (ho1s/rb1s/ng1s):
    Each variant is a SEPARATE energy_all refit on the instrument's feature
    subset. Training always uses the full 761-event pooled set; scoring uses
    ONLY the target instrument's slice (CPCV test rows in Phase 1; sealed-test
    rows in Phase 2).

Variants:
    cl1s:   full | pruned (C14_f11) | reduced (f11_move_z + f2_vol_60)
    ho1s:   full | pruned (F5_signal + C13_f11 + C6_f2)
                | reduced (7 raw + PC1+PC2 of C6_f2)
    rb1s:   full | pruned (F5_signal + C13_f11) | reduced (7 raw)
    ng1s:   full | pruned (C13_f11 + C9_f11_lowfreq_macro)
                | reduced (4 raw + PC1+PC2 of C9_f11_lowfreq_macro)
...
# Instrument dummies added by assemble_group for energy_all pool
# cl1s is the reference (dropped_first=True alphabetically)
_ENERGY_INST_DUMMIES = ["inst_ho1s", "inst_ng1s", "inst_rb1s"]

# Dummy values per target instrument (cl1s = reference → all zeros)
_INST_DUMMY_VALUES: dict[str, dict[str, float]] = {
...
INST_CONFIG: dict[str, dict] = {
    "cl1s": {"family": "xgb",      "pool": "individual"},
    "ho1s": {"family": "logistic",  "pool": "energy_all"},
    "rb1s": {"family": "logistic",  "pool": "energy_all"},
    "ng1s": {"family": "rf",        "pool": "energy_all"},
}

# Simplicity order: index 0 = most regularised
VARIANT_SPECS: dict[str, dict[str, dict]] = {
    "cl1s": {
        "full":    {"mode": "raw_all"},
        "pruned":  {"mode": "raw_clusters", "clusters": ["C14_f11"]},
        "reduced": {"mode": "raw_select",   "features": ["f11_move_z", "f2_vol_60"]},
    },
    "ho1s": {
        "full":   {"mode": "raw_all"},
        "pruned": {"mode": "raw_clusters", "clusters": ["F5_signal", "C13_f11", "C6_f2"]},
        "reduced": {
            "mode": "pca_plus_raw",
            "pca_specs": [{"cluster": "C6_f2", "n_components": 2}],
            "raw_features": [
                "f5_signal", "f5_long_bias_20", "f11_move_z", "f7_oi_price_div_20",
                "f11_ism_pmi_3m_change", "f1_bb_pctb_20", "f1_mr_score_20",
            ],
        },
    },
    "rb1s": {
        "full":   {"mode": "raw_all"},
        "pruned": {"mode": "raw_clusters", "clusters": ["F5_signal", "C13_f11"]},
        "reduced": {
            "mode": "raw_select",
            "features": [
                "f5_signal", "f5_long_bias_20", "f11_move_z", "f7_oi_price_div_20",
                "f2_vol_ratio_20_60", "f1_bb_pctb_20", "f11_ism_pmi_level",
            ],
        },
    },
    "ng1s": {
        "full":   {"mode": "raw_all"},
        "pruned": {"mode": "raw_clusters", "clusters": ["C13_f11", "C9_f11_lowfreq_macro"]},
        "reduced": {
            "mode": "pca_plus_raw",
            "pca_specs": [{"cluster": "C9_f11_lowfreq_macro", "n_components": 2}],
            "raw_features": [
                "f11_move_z", "hmm_macro_entropy", "f11_ng_stock_surprise", "f7_oi_level",
            ],
        },
    },
}
...
# ── Variant transform (identical to equity version) ───────────────────────────
...
def transform_variant(
    ...
    if mode == "raw_clusters":
        members = (
            cluster_df[cluster_df["cluster"].isin(spec["clusters"])]["feature"].tolist()
        )
...
def run_cpcv_individual(
    ...
    """CPCV for cl1s (individual instrument, XGB)."""
    ...
    for path_i, (tr_idx, te_idx) in enumerate(cpcv.split(ev_meta)):
        ...
        try:
            ...
            if family == "xgb":
                ...
                raise ValueError(f"Unexpected family for {inst}: {family}")
        ...
        for i in range(len(te_idx)):
            oos_rows.append({
                "date": ev_te.iloc[i]["date"], "y_true": int(y_te[i]),
...
# ── Pooled CPCV (ho1s / rb1s / ng1s via energy_all) ──────────────────────────

def run_cpcv_pooled(
    target_inst: str,
    variant: str,
    energy_all_events: pd.DataFrame,
    spec: dict,
    cluster_df: pd.DataFrame,
) -> tuple[pd.DataFrame, list[float]]:
    """CPCV for pooled energy model scored on target_inst slice.

    Trains on the full energy_all pool per CPCV fold.
    Scores only on target_inst's test rows within each fold.
    """
    fc = _feat_cols(energy_all_events)
    X_all = energy_all_events[fc].fillna(0.0).to_numpy(dtype=np.float64)
    y_all = energy_all_events["bin"].to_numpy(dtype=int)
    ev_meta = energy_all_events[["date", "t1", "bin", "instrument", "avg_uniqueness"]].copy()
    inst_arr = energy_all_events["instrument"].values
    ...
    for path_i, (tr_idx, te_idx) in enumerate(cpcv.split(ev_meta)):
        # Target instrument's slice of the test set
        te_target_mask = inst_arr[te_idx] == target_inst
        te_target_pos = te_idx[te_target_mask]

        if len(te_target_pos) < 2:
            n_skipped += 1
            continue
        y_te = y_all[te_target_pos]
        ...
        ev_te_target = ev_meta.iloc[te_target_pos].reset_index(drop=True)
        for i in range(len(te_target_pos)):
            oos_rows.append({
                "date": ev_te_target.iloc[i]["date"], "y_true": int(y_te[i]),
                "y_score": float(prob[i]), "path": path_i,
            })

    if n_skipped > 0:
        print(f"    [{target_inst}/{variant}] skipped {n_skipped}/15 paths (thin slice / single class)")
...
def select_locked_variant(cpcv_rows: list[dict]) -> str:
    """1SE rule: simplest within 1SE of full."""
...
def _load_test_individual(inst: str, data: dict, w_rets: pd.DataFrame,
                          train_feat_cols: list[str]) -> pd.DataFrame:
    """Load sealed test events for an individual instrument."""
    _, full_ev = build_feature_matrix(inst, data, w_rets)
    te = split_config.apply_test_mask(full_ev)
    if te.empty:
        return pd.DataFrame()
    return te.reindex(columns=list(_META) + train_feat_cols, fill_value=0.0)


def _load_test_pooled(target_inst: str, data: dict, w_rets: pd.DataFrame,
                      energy_feat_cols: list[str]) -> pd.DataFrame:
    """Load sealed test events for target instrument; add energy_all dummies."""
    _, full_ev = build_feature_matrix(target_inst, data, w_rets)
    te = split_config.apply_test_mask(full_ev)
    if te.empty:
        return pd.DataFrame()
    dummy_vals = _INST_DUMMY_VALUES.get(target_inst, {})
    for col, val in dummy_vals.items():
        te[col] = val
    return te.reindex(columns=list(_META) + energy_feat_cols, fill_value=0.0)


def refit_score_individual(
    inst: str, variant: str,
    train_events: pd.DataFrame, test_events: pd.DataFrame,
    spec: dict, cluster_df: pd.DataFrame,
) -> dict[str, Any]:
    """Phase 2: refit on full train, predict once on test (individual model)."""
    ...
    family = INST_CONFIG[inst]["family"]
    return _fit_and_score(inst, variant, X_tr_raw, y_tr, ev_tr, X_te_raw, y_te, fc, spec, cluster_df, family)


def refit_score_pooled(
    target_inst: str, variant: str,
    energy_all_train: pd.DataFrame, test_events: pd.DataFrame,
    spec: dict, cluster_df: pd.DataFrame,
) -> dict[str, Any]:
    """Phase 2: refit on full energy_all train, predict on target instrument test."""
    fc = _feat_cols(energy_all_train)
    X_tr_raw = energy_all_train[fc].fillna(0.0).to_numpy(dtype=np.float64)
    y_tr = energy_all_train["bin"].to_numpy(dtype=int)
    ev_tr = energy_all_train[["date","t1","bin","instrument","avg_uniqueness"]].reset_index(drop=True)
    X_te_raw = test_events.reindex(columns=fc, fill_value=0.0).fillna(0.0).to_numpy(dtype=np.float64)
    y_te = test_events["bin"].to_numpy(dtype=int)
    family = INST_CONFIG[target_inst]["family"]
    return _fit_and_score(target_inst, variant, X_tr_raw, y_tr, ev_tr, X_te_raw, y_te, fc, spec, cluster_df, family)
...
def _cpcv_chart(cpcv_rows: list[dict], locked: str, inst: str, out_path: Path) -> None:
    ...
    lo = max(0.3, min(valid) - 3 * max((s or 0) for s in stds))
    hi = min(1.0, max(valid) + 3 * max((s or 0) for s in stds))
    ax.set_ylim(lo, hi)
    ax.set_xlabel("Variant")
    ax.set_ylabel("AUC (mean ± std across usable CPCV paths)")
...
def _oos_summary_chart(oos_rows: list[dict], locked_picks: dict[str, str],
    ...
    ax.set_title("Energy — Phase 2 OOS (locked variants)")
...
def run(force: bool = False, phase1_only: bool = False) -> None:
    ...
    energy_all_train = pd.read_parquet(CACHE_DIR / "energy_all_events.parquet")
    cl1s_fc = _feat_cols(cl1s_train)
    ea_fc   = _feat_cols(energy_all_train)
    print(f"  cl1s:       {len(cl1s_train)} train events, {len(cl1s_fc)} features")
    print(f"  energy_all: {len(energy_all_train)} train events, {len(ea_fc)} features")
    for inst in ["ho1s", "rb1s", "ng1s"]:
        n = (energy_all_train["instrument"] == inst).sum()
        print(f"    {inst}: {n} events in pool")

    # ── Cluster membership ────────────────────────────────────────────────────
    cluster_dfs = {
        inst: load_cluster_membership(inst) for inst in INST_CONFIG
    }
    ...
    if cpcv_path.exists() and not force:
        ...
        for inst, spec_dict in VARIANT_SPECS.items():
            ...
            events = cl1s_train if pool == "individual" else energy_all_train

            for variant, spec in spec_dict.items():
                print(f"\n  [{inst}/{variant}] CPCV …")
                if pool == "individual":
                    oos_df, path_aucs = run_cpcv_individual(inst, variant, events, spec, cd)
                else:
                    oos_df, path_aucs = run_cpcv_pooled(inst, variant, events, spec, cd)
    ...
    for inst in INST_CONFIG:
        ...
        for r in inst_rows:
            print(f"    {r['variant']:8s}  AUC {r['auc_mean']:.4f} ± {r['auc_std']:.4f}  n_paths={r['n_paths']}")
    ...
    for inst in INST_CONFIG:
        inst_rows = cpcv_df[cpcv_df["inst"] == inst].to_dict("records")
        _cpcv_chart(inst_rows, locked_picks[inst], inst,
    ...
    if oos_path.exists() and not force:
        ...
        for inst, spec_dict in VARIANT_SPECS.items():
            ...
            if pool == "individual":
                te = _load_test_individual(inst, data, w_rets, cl1s_fc)
            else:
                te = _load_test_pooled(inst, data, w_rets, ea_fc)
            if te.empty:
            ...
            for variant, spec in spec_dict.items():
                ...
                if pool == "individual":
                    res = refit_score_individual(inst, variant, cl1s_train, te, spec, cd)
                else:
                    res = refit_score_pooled(inst, variant, energy_all_train, te, spec, cd)
    ...
    for inst in INST_CONFIG:
        ...
        print(f"  {inst:6s} ({locked:8s})  dev={dev:.4f}  OOS={oos_auc:.4f}  gap={dev-oos_auc:+.4f}")
