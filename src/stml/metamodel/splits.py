...
splits.py
=========
Chronological train/val/test partitioning plus the leakage controls used by the
feature-engineering layer to pin the **FE-train** partition (the window on which
fitted/TF feature families are fit) and the per-instrument embargo widths.

.. note::
   This module was **vendored** from the (now-removed) signal-replication
   package so the feature layer has no dependency on modelling/replication code.
   It is self-contained (numpy/pandas only). The ``feature-base`` branch keeps
   only the feature-engineering foundation; downstream branches add their own
   modelling. See ``reports/feature-catalog.md`` for how the FE-train boundary
   is used.
...
Two ideas drive this module:
  ...
  2. **Embargo the val edges so no constant-signal run straddles a boundary.**
     ...
     *structural* gap choice (standard purge/embargo practice): it leaks no
     return/label information -- only performance and threshold *calibration*
     must stay strictly train-only.
...
constant-value runs, i.e. the number of independent regime decisions.

**Implementation note (gating uses the POST-embargo window).** The ``n_eff``
that gates an instrument is computed on the *embargoed* val window::

    gateable_n_eff = n_eff(signal.iloc[embargoed_val(signal, split)])

As a defensive guard, if an (explicit) embargo is so wide that
``2 * embargo >= len(val)``, ``embargoed_val`` returns an empty array, which
callers must treat as "no leakage-free regime-call available", not an error.

:func:`get_test` is a deliberate tripwire so the test block is never touched
during development.
"""
...
def n_eff(signal_series: pd.Series | np.ndarray | list) -> int:
    ...
    Per the implementation note, the *gateable* ``n_eff`` for an instrument's
...
def embargoed_val(
    ...
    embargo : rows to drop at each edge. If ``None`` (default) it is
        ...
        instrument tends to hold a constant value.
    ...
    if embargo is None:
        ...
        # docstring): stable, and leaks no return/label info.
