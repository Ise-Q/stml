...
**The finding, in one line.** On the six-month out-of-sample window the pooled net Sharpe ratio of 0.48 is not statistically distinguishable from zero (*t* = 0.34, n = 127; bootstrap 95% interval [−0.09, +0.15], containing zero), and four further independent diagnostics agree: the conclusion is *insufficient evidence of a deployable edge*, not a proven failure. Under the modelling-committed per-class barrier the Energy book carries no signal and correctly abstains, taking no out-of-sample positions.
