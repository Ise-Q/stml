...
print(f"{'Instrument':10s} {'Signal':10s} {'Locked':8s} "
      f"{'Dev AUC':>8s} {'Dev AP':>7s} {'OOS AUC':>8s} {'OOS AP':>7s} {'OOS 95% CI':^16s} {'Gap':>7s}")
print("-" * 92)
for inst in ["gc1s", "si1s", "pl1s", "hg1s"]:
    lv = locked[inst]
    s  = sel.loc[inst]
    ...
    gap  = dev_auc - oos_auc
    print(f"{inst:10s} {signal:10s} {lv:8s} "
          f"{dev_auc:8.4f} {dev_ap:7.4f} {oos_auc:8.4f} {oos_ap:7.4f} {ci:^16s} {gap:+7.4f}")
"""
